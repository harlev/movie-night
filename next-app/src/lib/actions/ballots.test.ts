...
test('submitBallotAction resolves generic survey ownership and validates option ids', () => {
  const filePath = path.join(process.cwd(), 'src/lib/actions/ballots.ts');
  const source = readFileSync(filePath, 'utf8');

  assert.equal(source.includes('resolveSurveyBallotOwner'), true);
  assert.equal(source.includes("formData.get('guestName')"), true);
